...
@section('content')
    <div class="flex items-center justify-between">
        ...
        <a href="{{ url('/admin/feeds') }}" class="no-underline text-white px-4 my-3 mx-1 flex items-center custom-green py-1 justify-center">
...
<div class="flex flex-wrap justify-between">
  ...
  <div>
    ...
    <ul class="list-reset leading-loose py-2">
      <li class="text-sm py-1"><a href="{{url('/admin/feed/filter?search=achievement')}}">Achivements / Kudos</a></li>
      <li class="text-sm py-1"><a href="{{url('/admin/feed/filter?search=birthday')}}" value="birthday">Birthdays</a></li>
      <li class="text-sm py-1"><a href="{{url('/admin/feed/filter?search=workanniversary')}}" value="workanniversary">Work Anniversaries</a></li>
      <li class="text-sm py-1"><a href="{{url('/admin/feed/filter?search=teacher')}}" value="teacher">By Teacher / Staff</a></li>
      <li class="text-sm py-1"><a href="{{url('/admin/feed/filter?search=student')}}" value="student">By Students</a></li>
  ...
  <div class="py-3">
    ...
    <ul class="list-reset leading-loose py-2">
      <li class="text-sm py-1"><a href="{{url('/admin/feed/filter?list=school')}}" value="school">School</a></li>
      <li class="text-sm py-1"><a href="{{url('/admin/feed/filter?list=all_class')}}" value="all_class">All Classes</a></li>
      <li class="text-sm py-1"><a href="{{url('/admin/feed/filter?list=select_class')}}" value="select_class">My Class Only</a></li>
 ...
 @if(count($feeds)!=NULL)
    ...
    <div class="">
      ...
      <div> 
      <show-feed url="{{ url('/') }}" id="{{ $feed->id }}" mode="admin" auth_id="{{ \Auth::id() }}"></show-feed> 
  ...
  <div class="w-full lg:w-1/6 md:w-full">
    ...
    <div class="py-3 bg-white border border-gray-400 rounded px-3 my-2">
        ...
        @foreach($tags as $tag)
          ...
          <li class="px-2 py-1 border mr-1 my-1"><a href="{{url('/admin/feeds?search='.$tag->tag_name)}}">{{$tag->tag_name}}</a>({{$tag->getTag($tag->tag_name)}})</li>
          @endif
        @endforeach
        </ul>
    </div>
