        ...
        <div class="flex flex-wrap lg:flex-row justify-between">
            <div>
                <h1 class="admin-h1 mb-5 flex items-center">
                    <a href="{{ url('/teacher/studentLeaves') }}" title="Back" class="rounded-full bg-gray-300 p-2">
                    ...
                    <span class="mx-3">Approve Leave Application</span>
                </h1>
            </div>
        </div>
        @include('partials.message')
        <approve-student-leave url="{{ url('/') }}" id="{{ $leave->id }}" status="approve"></approve-student-leave>
    </div>
@endsection