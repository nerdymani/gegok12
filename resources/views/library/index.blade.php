    ...
    <div class="relative">
        <div class="flex flex-wrap lg:flex-row justify-between my-3">
            ...
            <div class="w-full lg:w-2/4">
   	            ...
   	            <portal-target name="LibraryMemberFilter" ></portal-target>
            </div>
        ...
        <form action="{{ url('/library/students') }}" enctype="multipart form-data">
        ...
        <librarymember-list url="{{ url('/') }}" searchquery="{{ $query }}" letter="{{ $alphabet }}" standard="{{ $standard }}" birthday="{{ $birthday }}" selected_standard="{{ $selected_standard }}"></librarymember-list>
        <librarymembersearch-filter url="{{ url('/') }}" searchquery="{{ $query }}" selected_standard="{{ $selected_standard }}"></librarymembersearch-filter>
    </div>
@endsection